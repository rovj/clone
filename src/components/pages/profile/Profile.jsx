...
function Profile() {
  ...
  const setToastError = (err) => {
    ...
    setTimeout(function () {
      snackRef.current.className = snackRef.current.className.replace(
        "showSnackbar",
        ""
      );
    }, 3000);
  ...
  const updateProfilePicture = async () => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `/users/${contextObj.user.docId}/ProfileImage`
      );
  ...
  let [followState, followDispatch] = useReducer(
    followReducer,
    followInitialState
  );
  ...
  useEffect(() => {
    if (contextObj.user != null) {
      ...
      const unsub2 = onSnapshot(
        doc(firestore, "users", contextObj.user.docId),
        (doc) => {
  ...
  return (
    <>
      ...
      >
        ...
        />
        <ChangeModal
          open={editState.isOpenEdit}
          resetEdit={resetEdit}
          state={editState.isState}
        >
          <>
                ...
                {loader ? (
                  ...
                  <div className="edit-menu">
                    <p
                      ...
                      onClick={() => editDispatch({ type: "name" })}
                    >
                      Change Name
                    </p>
                    <p
                      className="edit-menu-item"
                      onClick={() => editDispatch({ type: "email" })}
                    >
                      Change Email
                    </p>
                    <p
                    ...
                    <p
                      className="edit-menu-item cancel-item"
                      onClick={resetEdit}
                    >
                      Cancel
                    </p>
